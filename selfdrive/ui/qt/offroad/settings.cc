...
*/
  ...
  offroad_btns.append(new ButtonControl("Uninstall " + brand, "Run", "", [=]() {
    if (ConfirmationDialog::confirm("Are you sure you want to uninstall?", this)) {
      Params().putBool("DoUninstall", true);
    }
  }, "", this));
  offroad_btns.append(new ButtonControl("Review Training Guide", "Run",
  ...
  offroad_btns.append(new ButtonControl("Driver Camera", "Run",
                                        "Preview the driver facing camera to help optimize device mounting position for best driver monitoring experience. (vehicle must be off)",
                                        [=]() { emit showDriverView(); }, "", this));

  // calibration buttons
  QString resetCalibDesc = "(pitch) ↕ 5˚ \n(yaw) ↔ 4˚";
  ButtonControl *resetCalibBtn = new ButtonControl("Reset Calibration", "Run", resetCalibDesc, [=]() {
